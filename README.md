...
Originally built for [Amp](https://ampcode.com) by [@ryancarson](https://x.com/ryancarson/status/2008548371712135632). This fork adapts Ralph for Claude Code with optional Gemini/Codex agent skills.
...
cp -r skills/gemini-agent ~/.claude/skills/
cp -r skills/codex-agent ~/.claude/skills/
```
...
| `skills/gemini-agent/` | Skill for using Gemini CLI (research, large context) |
| `skills/codex-agent/` | Skill for using Codex CLI (code generation, refactoring) |
...
Frontend stories should include browser verification in acceptance criteria. Ralph will use the `dev-browser` skill (if installed) or fall back to Playwright/Puppeteer to navigate to the page, interact with the UI, and confirm changes work.

**Recommended:** Install the dev-browser plugin for best results:
```bash
...
- [Original Amp version by Ryan Carson](https://x.com/ryancarson/status/2008548371712135632)
